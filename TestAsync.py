...
async def wait_until_stop(timeout=15, stable_count=10):
    start = time.time()
    stable = 0
...
# 前面執行的動作都不是 Script → IsScriptFinish 一直是 False → run_script() 的第一段 while 永遠卡住
# → 引入 Script 狀態機
async def run_script(script_name, timeout=40):
...
async def run_move_to_target(target, timeout=40):
    global IsBusy
    start = time.time()
    ...
    if(target == "020010"):
        wcs.move_to_target("240", "020010")
    else:
        wcs.move_to_target("240", target)
...
class AGV:
    ...
    async def target_script(self, target):
        await run_move_to_target(target)
        return self
...
async def main():
    ...
    # 啟動狀態監控
    worker = asyncio.create_task(status_worker())
    ...
    # await robot.wait_pos(10010)
    await robot.target_script("030010")
    await robot.target_script("010020")
    # await robot.turn_right90()
    ...
    # await robot.target_script("010030")
    # await robot.target_script("010010")
    # await wait_until_position(10010)
    print("全部腳本完成，準備停止監控")
    worker.cancel()
